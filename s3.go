...
func init() {
	config.RegisterConstructor("s3", S3BucketConfig{})
}

type S3BucketConfig struct {
	BucketName string `json:"bucket_name" doc:"Name of AWS S3 bucket"`
}

func (c S3BucketConfig) Validate() error {
	return nil
}

func (c S3BucketConfig) Create() (keyvalue.Bucket, error) {
	aws, err := New()
	if err != nil {
		return nil, errors.Wrapf(err, "AWS error")
	}
	s3, err := aws.S3()
	if err != nil {
		return nil, errors.Wrapf(err, "AWS S3 error")
	}
	b, err := s3.Bucket(c.BucketName)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to get bucket(%s)", c.BucketName)
	}
	return b, nil
}
...
func (a *myAws) S3() (keyvalue.Buckets, error) {
	a.Lock()
	defer a.Unlock()
	if a.s3 == nil {
		a.s3 = s3.New(a.sess)
...
func (buckets *s3Buckets) Bucket(name string) (keyvalue.Bucket, error) {
	buckets.Lock()
	defer buckets.Unlock()
	...
	//try to open existing bucket
	openRes, err := buckets.s3.GetBucketAcl(&s3.GetBucketAclInput{
		Bucket: aws.String(name),
	})
	if err != nil {
		//try to create a new bucket
		createRes, err := buckets.s3.CreateBucket(&s3.CreateBucketInput{
			Bucket: aws.String(name),
		})
		if err != nil {
			return nil, errors.Wrapf(err, "failed to create bucket(%s)", name)
		}
		log.Infof("Create res: %+v", createRes.String())
		...
		log.Debugf("Opened AWS S3 bucket(%s): %s", name, openRes.String())
	}
