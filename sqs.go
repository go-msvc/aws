...
import (
	...
	"github.com/go-msvc/utils/streaming"
)

func (a *myAws) SQS(qname string, receive bool) (streaming.EventStream, error) {
	a.Lock()
	defer a.Unlock()
	if a.sqs == nil {
	...
	es := &sqsEventStream{
		eventChan: make(chan streaming.Event),
		stopped:   false,
		sess:      a.sess,
		sqs:       a.sqs,
		qname:     qname,
...
type sqsEventStream struct {
	eventChan chan streaming.Event
	stopped   bool
...
// events are processed from this chan by consumers like Workflow.Run()
func (es sqsEventStream) Chan() <-chan streaming.Event {
	return es.eventChan
}
...
func (es sqsEventStream) PushStart(e streaming.Event) error {
	if err := e.Validate(); err != nil {
...
func (es sqsEventStream) PushContinue(e streaming.Event) error {
	if err := e.Validate(); err != nil {
		return errors.Wrapf(err, "invalid event")
	}
	if !e.IsCont() {
...
func (es sqsEventStream) push(e streaming.Event) error {
